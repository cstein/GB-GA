...
def setup() -> argparse.Namespace:
    ...
    conformer_settings.add_argument("--conf-cpus", dest="conformer_cpus", metavar="number", type=int, default=num_cpus, help="number of cpus to use when generating conformations. Default: %(default)s.")
...
def get_nrb_options(args: argparse.Namespace) -> Optional[NumRotBondsOptions]:
    if args.scale_nrb:
        return NumRotBondsOptions(args.nrb_target, args.nrb_sigma)
    return None


def get_logp_options(args: argparse.Namespace) -> Optional[LogPOptions]:
    if args.scale_logp:
...
def get_screening_options(args: argparse.Namespace) -> ScreenOptions:
    ...
    nrb_screen = None
    if hasattr(args, "scale_nrb"):
        nrb_screen = get_nrb_options(args)
...
def options(args: argparse.Namespace, ignore: bool = False) -> Tuple[GAOptions,
    ...
    scaling_options: ScreenOptions = get_screening_options(args)
    ...
    if args.glide_grid is None and args.smina_grid is None and not ignore:
